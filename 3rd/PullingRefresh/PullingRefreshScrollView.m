...
#import "PullingRefreshScrollView.h"
...
@interface PullingRefreshScrollView ()
- (void)scrollToNextPage;
@end

@implementation PullingRefreshScrollView
...
- (void)awakeFromNib
{
    CGRect rect = CGRectMake(0, 0 - CGRectGetHeight(self.frame), CGRectGetWidth(self.frame), CGRectGetHeight(self.frame));
...
- (void)ascrollViewDidEndDragging:(UIScrollView *)scrollView {
    ...
    if (_headerView.state == kPRStatePulling) {
        ...
        if (_pullingDelegate && [_pullingDelegate respondsToSelector:@selector(pullingscrollViewDidStartRefreshing:)]) {
            [_pullingDelegate pullingscrollViewDidStartRefreshing:self];
        }
        ...
        NSLog(@"%@---%d",_pullingDelegate,[_pullingDelegate respondsToSelector:@selector(pullingscrollViewDidStartLoading:)]);
        if (_pullingDelegate && [_pullingDelegate respondsToSelector:@selector(pullingscrollViewDidStartLoading:)]) {
            [_pullingDelegate pullingscrollViewDidStartLoading:self];
        }
    }
}

- (void)ascrollViewDidFinishedLoading {
    [self ascrollViewDidFinishedLoadingWithMessage:nil];
}
...
- (void)ascrollViewDidFinishedLoadingWithMessage:(NSString *)msg{
    ...
    if (_headerView.loading) {
        ...
        if (_pullingDelegate && [_pullingDelegate respondsToSelector:@selector(pullingscrollViewRefreshingFinishedDate)]) {
            date = [_pullingDelegate pullingscrollViewRefreshingFinishedDate];
        }
    ...
    //    if (_footerView.state == kPRStateLoading) {
        ...
        NSDate *date = [NSDate date];
        if (_pullingDelegate && [_pullingDelegate respondsToSelector:@selector(pullingscrollViewLoadingFinishedDate)]) {
            date = [_pullingDelegate pullingscrollViewRefreshingFinishedDate];
        }
...
- (void)launchRefreshing {
    ...
    [UIView animateWithDuration:kPRAnimationDuration animations:^{
        ...
        [self ascrollViewDidEndDragging:self];
    }];
}
#pragma mark -
